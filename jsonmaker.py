...
class Cooldown():
    ...
    async def notify(self,  userid, channel, modchannel, message):
        with open(f'./users/{userid}.json', 'r+') as f:
            ...
            if str(channel) in data['cooldowns']:
                found = data['cooldowns'][str(channel)]
                cooldown = "".join(data['cooldowns'][str(channel)])
                ti = datetime.strptime(found, '%x %X')
                timediff = now.replace(tzinfo=None) - ti
                dth = str(timediff)[1:2]
                hours = str(timediff)[8:15]
                replaced = hours[0:2].replace(":", "")
                total = int(replaced) + (int(dth)*24)
                await modchannel.send(f"{message.author.mention} has posted too early in {message.channel.mention}. \nLast post: {cooldown} Time between messages: {total} hours")
