...
class agecalc(ABC):
    # This needs to become a separate file; so that it can be reused.
    @abstractmethod
...
class dblookup(ABC):
    # This entire class? Needs to be removed.
    @abstractmethod
...
class lobby(commands.GroupCog, name="lobby"):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
    # All of these need to be updated and made slash commands, and try to combine as many as possible; less is more.
...
DOB: {exists.dob}""")
    ...
    async def dbremoveid(self, ctx, userid: int):
        try:
            ...
            await ctx.send("Removal failed")
    # Reformat needed; more modulation
    # Try to use buttons to let users through instead; will need events.py. These commands should be used as backup.
    # Reduce database usage to the bare minimum.
